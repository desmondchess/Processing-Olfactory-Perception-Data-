...
begin
	...
	mintycelery = smilestomol("C/C=C(\\C)/C1CCC(O1)(C)C")
end
...
url = "https://raw.githubusercontent.com/SimonEnsemble/intro_to_data_science_fall_2022/main/hw/scent/leffingwell/behavior.csv"

# ╔═╡ cad842d7-91ca-4fdf-9aaf-8dfcb693433d
download(url, joinpath("dez", "behavior.csv"))

# ╔═╡ 34eee1ec-853a-4e05-96b7-15c4f1d1f72c
readdir()

# ╔═╡ 56d78035-b5bb-4fea-a92a-491a8b9879fc
pwd()

# ╔═╡ f3e979de-22a7-43db-b137-dab0298fe14c
leff_beh = CSV.read("behavior.csv", DataFrame)
...
new_data = select(leff_beh, ["IsomericSMILES", "Labels"])
...
rename!(new_data, "IsomericSMILES" => "molecule", "Labels" => "odor_(leffingwell)")
...
# ╔═╡ 5bd5ce6f-e492-4fe4-bb62-5c52c19759a2
begin
leffing =replace.(new_data[:,"odor_(leffingwell)"], ['[',']',''',','] => "")
new_data[:,"odor_leffing"] = leffing	
end

# ╔═╡ 1cd7b4f8-5475-4490-8e14-5022b7a9bef4
select!(new_data, ["molecule","odor_leffing"])

# ╔═╡ c078f912-e788-4579-8a6e-9fad37f37d7c
leffingwell = new_data
...
# ╔═╡ 77ba3a87-932b-4553-acc3-0804d20023e9
begin
	confirm = combine(groupby(new_data, "molecule"), "odor_leffing" => length => :n_distinct_molecule)
	println(sum(confirm[:,"n_distinct_molecule"]))
end

# ╔═╡ b8e5bba2-edc7-430b-add6-b932a09e37de
md"Yes, the number of unique molecules in the data frame is unique in each row"
...
download("https://raw.githubusercontent.com/SimonEnsemble/intro_to_data_science_fall_2022/main/hw/scent/goodscents/behavior.csv", joinpath("dez01", "behavior2.csv"))

# ╔═╡ 1c1acb7a-3922-4856-bc84-095a563914e1
gd_data = CSV.read("behavior2.csv", DataFrame)
...
rename!(gd_data, "Tags" => "odor (Goodscents)")
...
begin
	download("https://raw.githubusercontent.com/SimonEnsemble/intro_to_data_science_fall_2022/main/hw/scent/goodscents/identifiers.csv", joinpath("dez01", "identifiers.csv"))
	identifiers = CSV.read("identifiers.csv", DataFrame)
end

# ╔═╡ 7c3be4df-b1aa-4d39-9a6d-eea8eadc35c3
begin
	download("https://raw.githubusercontent.com/SimonEnsemble/intro_to_data_science_fall_2022/main/hw/scent/goodscents/molecules.csv",joinpath("dez01", "molecules.csv"))
	molecules = CSV.read("molecules.csv", DataFrame)
end
...
# ╔═╡ 5c74124a-a1b5-4cd8-b913-6440c440e1ce
begin
	println(length(unique(identifiers[:, "TGSC ID"])))
	println(length(unique(gd_data[:, "TGSC ID"])))
	println(length(unique(identifiers[:, "CID"])))
	println(length(unique(molecules[:, "CID"])))
end

# ╔═╡ 74836930-53bf-4b84-84fe-d97823b00072
behaviors_w_CIDs = innerjoin(identifiers, gd_data, on = "TGSC ID")
...
# ╔═╡ e2c8c5a2-edca-4b3b-97fa-0d2555148446
filter!(row -> row["odor (Goodscents)"] != "set()",goodscents)
...
begin
	ncol = replace.(goodscents[:,"odor (Goodscents)"], ['{','}',''',','] => "")
	goodscents[:,"odor (goodscents)"] = ncol
	select!(goodscents,["molecule", "odor (goodscents)"])
end
...
length(unique(goodscents[:,"molecule"])) == nrow(goodscents)

# ╔═╡ ce964c45-b67e-4a7e-b300-c2c6073941f8
groupby(unique(goodscents, "molecule"), "molecule")[70]
...
# ╔═╡ ea0e39f0-5cc6-4e72-a288-2959bd5c9857
begin
	view_unique = unique(goodscents, "molecule")
	groupby(goodscents, "molecule")[4]
end

# ╔═╡ 8fda05c5-a918-4964-a05c-e7bd43189aba
grouped_mole = groupby(goodscents, "molecule")

# ╔═╡ 67c2601e-0d5a-4894-94da-a2d6d5f9e99f
begin
	goodscents_fixed = DataFrame(molecule = String[], odor_goodscents = String[])
	for x in range(start = 1, stop =length(grouped_mole))
		odor_goodscents = []
		for y in eachrow(grouped_mole[x])
			push!(odor_goodscents, y["odor (goodscents)"] * " ")
		end
		push!(goodscents_fixed, [grouped_mole[x][:,"molecule"][1], reduce(*, odor_goodscents)]) 
	end
end
...
begin
	nrow(unique(goodscents_fixed, "molecule")) == length(goodscents_fixed[:,"molecule"]) 
...
# ╔═╡ 57aefbe2-151d-4eb7-bb27-2b0e43b8df21
md"outerjoin basically takes each passed dataframes and creates an output that contains all rows for values of the keys that exist in any(both empty and filled) of the passed data frames."
...
# ╔═╡ af85dca9-cd07-4d73-a273-a86c51d0e00f
describe(data, :nmissing, length => :length)

# ╔═╡ 7b4b6d00-a48e-45a9-8373-d0a14e02e7cd
md"5715 molecules are present in the joined dataframe, data"

# ╔═╡ 526c3b02-0aca-413d-b45b-f8ed477e494c
begin
	countᵢ = 0.0
	for (leff,good) in zip(data[:, "odor_leffing"],data[:,"odor_goodscents"])
		
		if ismissing(leff) == true && ismissing(good) == true
			countᵢ += 1 
...
# ╔═╡ e358b2fd-cbc6-4c5c-ba47-3a141d329ff5
md"No :missing is present in both leffing and goodscents"

# ╔═╡ 173883bc-2f1a-4dae-8f24-38a4db867c66
begin
	count = 0.0
	for (leff,good) in zip(data[:, "odor_leffing"],data[:,"odor_goodscents"])
		
		if ismissing(leff) == false && ismissing(good) == true
			count += 1 
...
# ╔═╡ 10ac52ef-add6-4a62-912c-502d26493d01
md"1297 missing is present in goodscent but not in leffing"
...
# ╔═╡ 0d2395f3-88a9-492c-a1e8-b7b18399b001
data

# ╔═╡ 1f8bb47c-fceb-419d-a5a7-f77511caa361
begin
	odor = []
		replace!(data.odor_leffing, missing => "")
		replace!(data.odor_goodscents, missing => "")
end

# ╔═╡ 43136e8d-1d7d-401d-8304-0a342bb57e99
data
...
# ╔═╡ 68dc2313-e414-4f9b-a5e4-fcd8efe61249
data[:, "odor"] = data[:, "odor_leffing"] .*" ".* data[:, "odor_goodscents"]
...
# ╔═╡ fec29e74-c9fb-4d79-ae78-e75a5257abce
data[:, "chck"] =[unique(split(odor, " ", keepempty=false)) for odor in data[:, "odor"]]

# ╔═╡ 664a1da4-e753-4617-9dd6-09e7e1fea44d
md"👃 to avoid distraction, delete all columns from `data` except for `\"molecule\"` and `\"odor\"`.
"

# ╔═╡ 11b899c1-dbca-4c96-8a06-a67902d913fc
select!(data, ["molecule", "chck"])

# ╔═╡ 5dbf1a4d-60f1-4d34-947b-cf6408b4c903
rename!(data, "chck" => "odor")
...
# ╔═╡ 6f398bbe-ca30-4c13-94ab-1b01ff85249d
transform!(data, "odor" => (col -> length.(col)) => "# odor labels")
...
# ╔═╡ 30fb6123-a1e3-410f-9a47-4f7421392aa5
odor_label_counts[:,2]
...
# ╔═╡ a55eabc4-6581-432a-b336-03914c68629c
begin
	local fig = Figure()
	local ax = Axis(fig[1,1], title="Odor Frequencies Distribution", xlabel="# Molecules", ylabel="# of Odor Labels")

	barplot!( 1:21,odor_label_counts[:,2],  direction=:y)
	fig
end
...
begin
	for (x,y) in enumerate(data[:,"odor"])
		if "mint" in y
			if "celery" in y
				println(data[x,"molecule"])
			end
		end
	end
end

# ╔═╡ 7281afcf-6029-400d-9d4f-fc2fa086aff0
data[2004, "molecule"]

# ╔═╡ c2864dbf-84d3-41b1-b3f5-4319786867de
HTML(drawsvg(mintycelery, 200, 200))
...
unique_odor_labels = unique(reduce(vcat, data[:, "odor"]))

# ╔═╡ 75e8312d-1614-4626-99d1-3e07fdec8bab
odor_labels = reduce(vcat, data[:, "odor"])

# ╔═╡ 3d1f7cfa-ae9b-4ed1-9261-595fee65a974
begin
	new = length(odor_labels)
	empty = ""
	for i in 1:new
		empty = empty * " " * odor_labels[i]
	end
	empty
end
...
begin
	new₁ = length(unique_odor_labels)
	countᵢᵢ  = zeros(new₁)
	for i in 1:new₁
		countᵢᵢ[i] = length(collect(eachmatch(Regex(unique_odor_labels[i]), empty)))
	end
	countᵢᵢ
end

# ╔═╡ cb4e5e31-51ea-497d-b6af-585eb33b1061
length(unique_odor_labels)

# ╔═╡ 386b83dc-85f6-4283-bf3f-0078a3a9c03f
label_Den = DataFrame(odor=unique_odor_labels, count= countᵢᵢ)

# ╔═╡ 9bee24bb-4263-4b96-9c72-094b5ff8a182
begin
	sort!(label_Den, "count", rev=true)
	#delete!(label_Den, 1)
end

# ╔═╡ e30b3847-30a7-4464-83d9-245ab83b35d7
 label_Den[:, "odor"]

# ╔═╡ e24d1eb3-d861-4028-ba66-3962c76bc3d0
begin
	fig = Figure(resolution=(500, 10000))
	ax = Axis(fig[1,1], title="Label Density Distribution", ylabel="Odors Label", xlabel="# molecules", yticks =(1:length(label_Den[:, "odor"]), label_Den[:, "odor"]))

	barplot!(1:length((label_Den)[:,2]),label_Den[:, 2], direction=:x)
	xlims!(0, nothing)
	ylims!(0, length(label_Den[:, "odor"]))
	fig
end
...
# ╔═╡ 1a22a5db-2818-4e17-8171-ad2ff4e6d0fc
data[:,"molecule"]

# ╔═╡ 72101250-16ae-4c20-a7aa-e1479a41860f
begin
	count5 = 0
	for i in 1:length(data[:,"molecule"])
		if occursin("C(=O)", data[i, "molecule"])
			count5 += 1
		end
	end
	count5
...
# ╔═╡ 76dffd09-27aa-4aa4-91fc-260b815362f0
md"2921 odorants have a carbonyl group in their structure"
...
begin
	apple_count = 0
	fruity_count = 0
	for i in 1:length(data[:,"odor"])
		if confm("apple", data[i, "odor"]) == true
			apple_count += 1
			if confm("fruity", data[i, "odor"]) == true
				fruity_count += 1
			end
		end
	end
...
# ╔═╡ 648465bb-5a9c-4b31-851d-92486be8278d
[apple_count, fruity_count, fruity_count/apple_count]

# ╔═╡ c5600923-3d35-4695-b931-df0033aebc65
md"A strong co-occurance truly occurs among molecules with apple as its olfactory label and also tagged as fruity, about 90%!!"
...
begin
	...
	musk_count = 0
	for i in 1:length(data[:,"odor"])
		if confm("cabbage", data[i, "odor"]) == true
			cabbage_count += 1
			if confm("musk", data[i, "odor"]) == true
				musk_count += 1
			end
		end
	end
...
[cabbage_count, musk_count, musk_count/cabbage_count]

# ╔═╡ 7242a8c3-362c-4df9-b518-1a2a40ca2f39
md"While a weak co-occurance occurs among molecules with cabbage as its olfactory label and also tagged as musk, about 2%!!"
...
julia_version = "1.8.1"
manifest_format = "2.0"
project_hash = "dedd12be42f24384a3e0276b35d3d96b7a649385"
